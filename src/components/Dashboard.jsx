    ...
    <div className="animate-slide-up space-y-8">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-3xl font-bold text-foreground">
            Welcome back,{" "}
            <span className="text-accent">{mockUser.name.split(" ")[0]}</span>
          </h1>
        ...
        <div
          className={`flex items-center gap-2 rounded-full border border-neon-amber/30 bg-neon-amber/10 px-4 py-2 ${badge.color}`}
        >
          <Trophy className="h-5 w-5" />
          <span className="font-display text-sm font-bold tracking-wider">
            {mockUser.league.toUpperCase()} TIER
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard
          Icon={Coins}
          label="Battle Coins"
          value={mockUser.battleCoins.toLocaleString()}
        ...
        <StatCard
          Icon={TrendingUp}
          label="Win/Loss"
          value={`${mockUser.wins}W / ${mockUser.losses}L`}
          accent="text-accent"
